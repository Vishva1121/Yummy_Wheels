...
const Restaurant = require('./models/userModel').restaurant;
...
app.post("/registerRestaurant", function (req, res) {
  Restaurant.register({ username: req.body.username, name: req.body.name, ownerName: req.body.oName, phoneNumber: req.body.phoneNumber, category: req.body.category, menu: { price: 3, item: "Panner Tikka" } }, req.body.password, function (err, user) {
    if (err) {
      console.log(err);
...
app.get("/forCheck", function (req, res) {
  res.render("forCheck", { genderDetails: "/static/male-avatar.png" });
})
